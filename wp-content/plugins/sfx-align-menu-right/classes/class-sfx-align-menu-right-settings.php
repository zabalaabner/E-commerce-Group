...
/**
 * SFX_Align_Menu_Right_Settings Class
 *
 * @class SFX_Align_Menu_Right_Settings
 * @version	1.0.0
 * @since 1.0.0
 * @package	SFX_Align_Menu_Right
 * @author Alan
 */
final class SFX_Align_Menu_Right_Settings {
	/**
	...
	/**
	 * Main SFX_Align_Menu_Right_Settings Instance
	 *
	 * Ensures only one instance of SFX_Align_Menu_Right_Settings is loaded or can be loaded.
	 *
	 * @since 1.0.0
	 * @static
	 * @return Main SFX_Align_Menu_Right_Settings instance
	 */
		...
		if ( is_array( $input ) && 0 < count( $input ) ) {
			...
			foreach ( $input as $k => $v ) {
				...
				if ( ! method_exists( $this, $method ) ) {
					if ( true == (bool)apply_filters( 'sfx-align-menu-right-validate-field-' . $fields[$k]['type'] . '_use_default', true ) ) {
				...
				if ( '' != $method ) {
					add_filter( 'sfx-align-menu-right-validate-field-' . $fields[$k]['type'], array( $this, $method ) );
				}

				$method_output = apply_filters( 'sfx-align-menu-right-validate-field-' . $fields[$k]['type'], $v, $fields[$k] );
	...
	public function render_field ( $args ) {
		...
		// Construct the key.
		$key 				= SFX_Align_Menu_Right()->token . '-' . $args['section'] . '[' . $args['id'] . ']';
	...
	public function get_settings_sections () {
		...
		$settings_sections['standard-fields'] = __( 'Standard Fields', 'sfx-align-menu-right' );
		$settings_sections['special-fields'] = __( 'Special Fields', 'sfx-align-menu-right' );
		...
		return (array)apply_filters( 'sfx-align-menu-right-settings-sections', $settings_sections );
	...
	public function get_settings_fields ( $section ) {
		...
		switch ( $section ) {
			case 'standard-fields':
				...
				$settings_fields['text'] = array(
												'name' => __( 'Example Text Input', 'sfx-align-menu-right' ),
												'type' => 'text',
												'default' => '',
												'section' => 'standard-fields',
												'description' => __( 'Place the field description text here.', 'sfx-align-menu-right' )
											);
				$settings_fields['textarea'] = array(
												'name' => __( 'Example Textarea', 'sfx-align-menu-right' ),
												'type' => 'textarea',
												'default' => '',
												'section' => 'standard-fields',
												'description' => __( 'Place the field description text here.', 'sfx-align-menu-right' )
				...
				$settings_fields['checkbox'] = array(
												'name' => __( 'Example Checkbox', 'sfx-align-menu-right' ),
												'type' => 'checkbox',
												'default' => '',
												'section' => 'standard-fields',
												'description' => __( 'Place the field description text here.', 'sfx-align-menu-right' )
				...
				$settings_fields['radio'] = array(
												'name' => __( 'Example Radio Buttons', 'sfx-align-menu-right' ),
												'type' => 'radio',
												'default' => '',
												'section' => 'standard-fields',
												'options' => array(
																	'one' => __( 'One', 'sfx-align-menu-right' ),
																	'two' => __( 'Two', 'sfx-align-menu-right' ),
																	'three' => __( 'Three', 'sfx-align-menu-right' )
															),
												'description' => __( 'Place the field description text here.', 'sfx-align-menu-right' )
											);
				$settings_fields['select'] = array(
													'name' => __( 'Example Select', 'sfx-align-menu-right' ),
													'type' => 'select',
													'default' => '',
													'section' => 'standard-fields',
													'options' => array(
			...
			case 'special-fields':
				...
				$settings_fields['select_taxonomy'] = array(
													'name' => __( 'Example Taxonomy Selector', 'sfx-align-menu-right' ),
		...
		return (array)apply_filters( 'sfx-align-menu-right-settings-fields', $settings_fields );
	...
	public function get_supported_fields () {
		return (array)apply_filters( 'sfx-align-menu-right-supported-fields', array( 'text', 'checkbox', 'radio', 'textarea', 'select', 'select_taxonomy' ) );
	...
	public function get_value ( $key, $default, $section ) {
		$values = get_option( 'sfx-align-menu-right-' . $section, array() );
	...
	public function get_settings ( $section = '' ) {
		...
		if ( 0 < count( $sections ) ) {
			foreach ( $sections as $k => $v ) {
				...
				$values = get_option( 'sfx-align-menu-right-' . $v, array() );
