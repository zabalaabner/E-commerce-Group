...
function mp_emmet_customize_register( $wp_customize ) {
	 ...
	 */
	$wp_customize->add_control(
		'theme_pinterests_link', array(
			'label'    => esc_html__( 'Pinterest link', 'emmet-lite' ),
			'section'  => 'theme_header_socials',
			'settings' => 'theme_pinterest_link',
		)
	);/*
	 * Add the 'instagram link' setting.
	 */
	$wp_customize->add_setting(
		'theme_instagram_link', array(
			'capability'        => 'edit_theme_options',
			'transport'         => 'postMessage',
			'sanitize_callback' => 'esc_url_raw',
		)
	...
	$wp_customize->add_control(
		'theme_instagram_link', array(
			'label'    => esc_html__( 'Instagram link', 'emmet-lite' ),
			'section'  => 'theme_header_socials',
			'settings' => 'theme_instagram_link',
		)
	);/*
	 * Add the 'tumblr link' setting.
	 */
	$wp_customize->add_setting(
		'theme_tumblr_link', array(
			'capability'        => 'edit_theme_options',
			'transport'         => 'postMessage',
			'sanitize_callback' => 'esc_url_raw',
		)
	...
	$wp_customize->add_control(
		'theme_tumblr_link', array(
			'label'    => esc_html__( 'Tumblr link', 'emmet-lite' ),
			'section'  => 'theme_header_socials',
			'settings' => 'theme_tumblr_link',
		)
	);/*
	 * Add the 'youtube link' setting.
	 */
	$wp_customize->add_setting(
		'theme_youtube_link', array(
			'capability'        => 'edit_theme_options',
			'transport'         => 'postMessage',
			'sanitize_callback' => 'esc_url_raw',
		)
	...
	/*
	 * Add the upload control for the 'youtube link' setting.
	 */
	$wp_customize->add_control(
		'theme_youtube_link', array(
			'label'    => esc_html__( 'Youtube link', 'emmet-lite' ),
			'section'  => 'theme_header_socials',
			'settings' => 'theme_youtube_link',
		)
	);
	/*
	 * Add 'header_socials' section
	...
	$wp_customize->add_setting( 'theme_accent_description', array(
		'default'           => __( 'Install the theme in several clicks and start editing it with a help of a handy WordPress Customizer or use a very user-friendly built-in visual page builder.', 'emmet-lite' ),
		'capability'        => 'edit_theme_options',
		'sanitize_callback' => 'mp_emmet_sanitize_text',
		'transport'         => 'postMessage'
	) );
...
function mp_emmet_sanitize_text( $txt ) {
	return wp_kses( $txt, mp_emmet_allowed_html() );
}
...
function theme_customize_preview_js() {
	wp_enqueue_script( 'theme-customizer', get_template_directory_uri() . '/js/theme-customizer.min.js', array( 'customize-preview' ), mp_emmet_get_theme_vertion(), true );
}
...
function theme_customize_control_js() {
	wp_enqueue_script( 'color-scheme-control', get_template_directory_uri() . '/js/color-scheme-control.min.js', array(
		'customize-controls',
		'iris',
		'underscore',
		'wp-util'
...
function mp_emmet_choice_callback( $control ) {
	...
	if ( ( 
		     $control_id == 'theme_bigtitle_brandbutton_url' || $control_id == 'theme_bigtitle_brandbutton_label' || $control_id == 'theme_bigtitle_whitebutton_url' || $control_id == 'theme_bigtitle_whitebutton_label' || $control_id == 'theme_bigtitle_title' || $control_id == 'theme_bigtitle_description' ) && $radio_setting == 'd'
	) {
		return true;
	}
